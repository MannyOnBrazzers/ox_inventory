...
local function canOpenTarget(ped)
	...
	or IsEntityPlayingAnim(ped, "switch@trevor@annoys_sunbathers", "trev_annoys_sunbathers_loop_guy", 3)
end
...
local function canUseItem(isAmmo)
	...
	and not exports['rush-scripts']:isAimingInVehicle()
end
...
RegisterNUICallback('openPersonalPouch', function(_, cb)
	local cid = exports['brazzers-lib']:getCID()
	local id = 'personal_pouch_'..cid
    if not exports.ox_inventory:openInventory('stash', id) then
        local success = lib.callback.await('brazzers-inv:server:registerPersonalPouch', false, id)
