...
local function removeCurrency(inv, moneyType, price)
	exports['brazzers-lib']:removeShopMoney(inv, moneyType, price)
end
	...
	if data.toType == 'player' then
		...
		if fromData then
			...
			if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				local canAfford, moneyType = exports['brazzers-lib']:canAffordShopPurchase(source, price)
				...
				removeCurrency(source, moneyType, price)
